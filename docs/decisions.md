...
## In-ROM libgcc cluster `[0x08033ca4, 0x0803578c)` — link archive members directly

The libgcc run in this ROM is not decomp work. It is byte-identical to
members of `tools/agbcc/lib/libgcc.a`, and the final pattern is to link
those archive members at their baserom addresses in `linker.ld`.

Current linked members:

- `_ashldi3.o`, `_call_via_rX.o`, `_divsi3.o`, `_dvmd_tls.o`,
  `_fixunssfsi.o`, `_lshrdi3.o`, `_modsi3.o`, `_udivsi3.o`,
  `_umodsi3.o`
- `fp-bit.o` for single-precision soft-float
- `_muldi3.o`
- `dp-bit.o` for double-precision soft-float
- `_negdi2.o`

`fp-bit.o` and `dp-bit.o` also need their static BSS slots placed in
IWRAM at `0x03003440` and `0x03003450`. The linker script keeps old
`sub_XXXXXXXX` names as aliases only where project code still references
them.

Do not create C scaffolds, NAKED wrappers, or `.incbin` asm stubs for
these helpers. If a future peeled range byte-matches a libgcc archive
member, wire the member into `linker.ld`, add compatibility aliases if
needed, delete the peel/scaffold, run `make check`, and update the
agent notes so the picker does not offer it as a decomp target.
