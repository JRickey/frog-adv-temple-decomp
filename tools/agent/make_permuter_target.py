...
def hword(data: bytes, off: int) -> int:
    return data[off] | (data[off + 1] << 8)


def is_thumb_bl(first: int, second: int) -> bool:
    return (first & 0xf800) == 0xf000 and (second & 0xf800) == 0xf800


def normalize_thumb_call_relocs(data: bytes, relocs: dict[int, tuple[str, str]]) -> dict[int, tuple[str, str]]:
    """Normalize Thumb BL relocations to the first halfword of the instruction.

    Some binutils/object combinations report R_ARM_THM_CALL at the relocation
    halfword instead of the 32-bit Thumb instruction start. Emitting `bl sym`
    from that offset duplicates one halfword and corrupts the permuter target.
    """
    normalized = {}
    for off, (sym, typ) in relocs.items():
        reloc_off = off
        if typ == "R_ARM_THM_CALL":
            if off + 3 < len(data) and is_thumb_bl(hword(data, off), hword(data, off + 2)):
                reloc_off = off
            elif off >= 2 and off + 1 < len(data) and is_thumb_bl(hword(data, off - 2), hword(data, off)):
                reloc_off = off - 2
        normalized[reloc_off] = (sym, typ)
    return normalized
...
def main() -> int:
    ...
    cand = root_path(args.candidate) if args.candidate else find_candidate_o(fn)
    ...
    relocs = normalize_thumb_call_relocs(data, relocations(cand, start, size))
    ...
    out = [
        ...
        f"@ Mirrors the candidate ({display_path(cand)}) $t/$d mapping + relocations",
    ...
    out_dir = root_path(args.out) if args.out else (ROOT / "nonmatchings" / fn)
    ...
    print(f"wrote {display_path(tgt_s)} and {display_path(tgt_o)}")
