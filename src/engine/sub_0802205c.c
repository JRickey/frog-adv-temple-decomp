...
void LevelLayout_LoadSlots311074(void)
{
    ...
    for (i = 0; i < 14; i++) {
        if ((i & 1) == 0)
            Entity_InitSlotFromRecord(i + 25, (const void *)sLevelLayoutPtrs_311074[i], 0, 5, 0x61, 4, 3, 3);
        else
            Entity_InitSlotFromRecord(i + 25, (const void *)sLevelLayoutPtrs_311074[i], 0, 6, 0x71, 4, 3, 3);
    }
}
